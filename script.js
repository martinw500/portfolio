...
// Cursor-following effect for hero profile image
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (heroImg) {
        ...
        heroImg.addEventListener('mouseleave', () => {
            ...
            heroImg.style.clipPath = 'circle(140px at center)';
        });
        ...
        heroImg.addEventListener('mousemove', (e) => {
            ...
            // Normalize to percentage for clip-path positioning
            const percentX = 50 + (mouseX / rect.width) * 30; // 20% to 80% range
            const percentY = 50 + (mouseY / rect.height) * 30; // 20% to 80% range
            
            // Calculate distance from center to determine reveal amount
            const distance = Math.sqrt(mouseX * mouseX + mouseY * mouseY);
            const maxDistance = Math.sqrt((rect.width/2) * (rect.width/2) + (rect.height/2) * (rect.height/2));
            const normalizedDistance = Math.min(distance / maxDistance, 1);
            
            // Base radius + extra reveal based on cursor position
            const baseRadius = 140;
            const maxReveal = 40; // Maximum additional pixels to reveal
            const revealAmount = baseRadius + (maxReveal * (1 - normalizedDistance * 0.3));
            
            // Subtle movement towards cursor (max 4px)
            const moveX = (mouseX / rect.width) * 8;
            const moveY = (mouseY / rect.height) * 8;
            
            // Apply transforms
            heroImg.style.transform = `translateY(${-6 + moveY}px) scale(1.02) translateX(${moveX}px)`;
            heroImg.style.clipPath = `circle(${revealAmount}px at ${percentX}% ${percentY}%)`;
        });
    }
});
